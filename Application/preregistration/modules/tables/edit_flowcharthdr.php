<?php
//****************************************************************************************
//Generated by Cobalt, a rapid application development framework. http://cobalt.jvroig.com
//Cobalt developed by JV Roig (jvroig@jvroig.com)
//****************************************************************************************
require 'path.php';
init_cobalt('Edit flowcharthdr');

if(isset($_GET['flow_id']))
{
    $flow_id = urldecode($_GET['flow_id']);
    require 'form_data_flowcharthdr.php';

}

if(xsrf_guard())
{
    init_var($_POST['btn_cancel']);
    init_var($_POST['btn_submit']);
    require 'components/query_string_standard.php';
    require 'subclasses/flowcharthdr.php';
    $dbh_flowcharthdr = new flowcharthdr;

    $object_name = 'dbh_flowcharthdr';
    require 'components/create_form_data.php';

    extract($arr_form_data);

    if($_POST['btn_cancel'])
    {
        log_action('Pressed cancel button');
        redirect("listview_flowcharthdr.php?$query_string");
    }


    if($_POST['btn_submit'])
    {
        log_action('Pressed submit button');

        $message .= $dbh_flowcharthdr->sanitize($arr_form_data)->lst_error;
        extract($arr_form_data);

        if($dbh_flowcharthdr->check_uniqueness_for_editing($arr_form_data)->is_unique)
        {
            //Good, no duplicate in database
        }
        else
        {
            $message = "Record already exists with the same primary identifiers!";
        }

        if($message=="")
        {
            $dbh_flowcharthdr->edit($arr_form_data);


            redirect("listview_flowcharthdr.php?$query_string");
        }
    }
}
require 'subclasses/flowcharthdr_html.php';
$html = new flowcharthdr_html;
$html->draw_header('Edit Flowcharthdr', $message, $message_type);
$html->draw_listview_referrer_info($filter_field_used, $filter_used, $page_from, $filter_sort_asc, $filter_sort_desc);
$html->draw_hidden('flow_id');

$html->draw_controls('edit');

$html->draw_footer();